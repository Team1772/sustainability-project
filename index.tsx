...
const App: React.VFC = () => {
  ...
  const [loc, setLoc] = React.useState<google.maps.LatLngLiteral>({
    lat: 0,
    lng: 0
  })
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      >
        ...
        >
          {trashes.map((trash: Trash, i) => (
            <Marker
              key={i}
              position={trash.latLng}
              icon={trashTypeImage[trash.type]}
            />
          ))}
          <Marker position={loc} icon='./icons/user.png' />
        </Map>
      </Wrapper>
      ...
      <div id='fixed-bar'>
        ...
        >
          ...
          >
            {trash.type
              ? `Set ${trash.type} wastebin to current location`
              : 'Select a wastebin'}
          </button>
        </div>
      </div>
...
const Marker: React.FC<google.maps.MarkerOptions & { icon: string }> = ({
  icon,
  ...markerOptions
}) => {
